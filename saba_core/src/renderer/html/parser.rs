        ...
        while token.is_some() {
            match self.mode {
                ...
                InsertionMode::Initial => {
                    ...
                    // <!DOCTYPE html> is represented as Token::Char('!') through Token::Char('l').
                ...
                InsertionMode::InHead => {
                    match token {
                        ...
                        Some(HtmlToken::StartTag {
                            ...
                            // This is not defined in the spec but we need this for HTML without <head>
                            // Otherwise, infinite loop occurs when <head> tag doesn't exist.
                            if tag == "body" {
                                self.pop_until(ElementKind::Head);
                                self.mode = InsertionMode::AfterHead;
                                continue;
                            }
                            if let Ok(_element_kind) = {
                                let s: &str = tag;
                                match s {
                    ...
                    // ignore unsupported tag like <meta> | <title>
                    token = self.t.next();
                    continue;
                }
                            ...
                            match tag.as_str() {
                                ...
                                "p" => {
                                    let element_kind = ElementKind::from_str(tag)
                                        .expect("failed to convert to ElementKind");
                                    token = self.t.next();
