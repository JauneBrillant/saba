    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let token = match c {
                ...
                '#' => {
                    // #は、常に#ID形式のIDセレクタとして扱う
                    //　他にhexcodeで表される色や、urlのフラグメントとしての#が考えられるが、本実装では未対応
                    let value = self.consume_ident_token();
                    self.pos -= 1;
                    CssToken::HashToken(value)
                }
                '-' => {
                    // 本実装では負の数は未対応、ハイフンは識別子の一つとして扱う
                    let t = CssToken::Ident(self.consume_ident_token());
                    self.pos -= 1;
                    t
                }
                '@' => {
                    // 次の３文字が識別子として有効な文字の場合、<at-keyword-token>トークンを返す
...
mod tests {
    ...
    fn test_one_rule() {
        ...
        for e in expected.iter() {
            assert_eq!(Some(e), t.next().as_ref());
        }
        assert!(t.next().is_none());
    }
    ...
    fn test_multiple_rules() {
        ...
        let expected = [
            ...
            CssToken::OpenCurly,
            CssToken::Ident("font_size".to_string()),
            CssToken::Colon,
