...
impl CssParser {
    ...
    fn consume_list_of_rules(&mut self) -> Vec<QualifiedRule> {
        ...
        loop {
            ...
            match token {
                // AtKeywordトークンが出てきた場合、@import, @mediaなどのルールが始まる事を示す
                // 本実装は@から始まるルールはサポートしない
    ...
    fn consume_declaration(&mut self) -> Option<Declaration> {
        ...
        // もし次のトークンがコロンでない場合、パースエラーなのでNoneを返す
        match self.t.next() {
...
mod tests {
    ...
    fn test_one_rule() {
        ...
        assert_eq!(cssom.rules, expected);
    }
    ...
    fn test_multiple_rules() {
        ...
        let rule1 = QualifiedRule {
            selector: Selector::TypeSelector("p".to_string()),
            declarations: vec![Declaration {
                property: "content".to_string(),
                value: ComponentValue::StringToken("Hey".to_string()),
            }],
        };
        let rule2 = QualifiedRule {
            selector: Selector::TypeSelector("h1".to_string()),
            declarations: vec![
                Declaration {
                    property: "font-size".to_string(),
                    value: ComponentValue::Number(40.0),
                },
                Declaration {
                    property: "color".to_string(),
                    value: ComponentValue::Ident("blue".to_string()),
                },
            ],
        };
